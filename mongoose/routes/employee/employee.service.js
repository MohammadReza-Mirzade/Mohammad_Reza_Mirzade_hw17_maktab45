...
const updateEmployee = (req, res) => {
    ...
    if (req.body.beManager === 'yes') {
        Company.findOne({registrationNumber: req.body.company}, (err, company) => {
            if (err) return res.status(500).json({msg: "Server Error :)", err: console.log(err.message)});
            if (!company) return res.status(406).render('message', {message: "Company doesn't exist :("});
            Employee.findOne({$and: [{company: company._id}, {beManager: true}, {_id: {$ne: req.params.id}}]}, (err, employee) => {
                if (err) return res.status(500).json({msg: "Server Error :)", err: console.log(err.message)});
                if (employee) return res.status(406).render('message', {message: 'This Company already have a manager if you want to change manager at first you must change the manager property.'});
            });
            const updateObj = {
                lastName: req.body.lastName,
                firstName: req.body.firstName,
